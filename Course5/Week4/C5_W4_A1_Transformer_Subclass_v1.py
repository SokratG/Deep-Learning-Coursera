...
def scaled_dot_product_attention(q, k, v, mask):
    ...
    # scale matmul_qk
    dk = tf.math.sqrt(tf.cast(k.shape[0], tf.float32))
...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, training, mask):
        ...
        self_attn_output = self.mha(query=x, value=x, attention_mask=mask)  # Self attention (batch_size, input_seq_len, embedding_dim)
